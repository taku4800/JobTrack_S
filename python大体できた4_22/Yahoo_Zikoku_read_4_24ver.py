...
def imagecut(image,square):
    ...
    r,g,b,A = alpha_image.getpixel((1,1))#ダークモードが判定
    RGB=r+g+b
    print("RGBの和",RGB,"\n")
    if  RGB<300:#ダークモードで黒いときだけ反転
        alpha_image = ImageOps.invert(alpha_image.convert('RGB'))#色を逆転する 清水のがダークモードで白字だったので黒字になる
    cutimage = alpha_image.convert('L')#白黒にする
    """
...
print("長さは")
print((len(matches_cleaned)))
for i in matches_cleaned:
    print(i)
    pass
